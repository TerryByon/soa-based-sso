...
public class AskUserDao extends HibernateSessionSupport implements IAskUserDao {
	...
	public void deleteUser(User user) {
		getSession().createQuery("delete from User where serverid = :serverid and userid=:userid").setInteger("serverid",
				user.getServerid()).setString("userid", user.getUserid()).executeUpdate();
	...
	public void deleteUserBeforeGet(String[] ticketInfo) {
		getSession().createQuery("delete from User where serverid = :serverid and userid=:userid and validtime < :validtime").setInteger(
				"serverid", Integer.valueOf(ticketInfo[0])).setString("userid", ticketInfo[2]).setString("validtime",
				DateUtil.getNow("yyyyMMddHHmmssSSS")).executeUpdate();
