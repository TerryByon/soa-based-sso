...
import com.apex.sso.askuser.IAskUserController;
	...
	public void getUser(@RequestParam("ticket") String ticket,HttpServletRequest request, HttpServletResponse response){
		...
		try {
			...
			try{
				...
				int timeDef = DateUtil.getDateDiff(DateUtil.getNow("yyyyMMddHHmmssSSS"), ticketInfo[3], DateUtil.DATE_INTERVAL_SECOND, "yyyyMMddHHmmssSSS",false);
				if(timeDef>0){
					manager.deleteUser(user);
					//세션 끊김
					out.println("<result>flase</result>");
...
//		String date17 = DateUtil.dateAdd(16, 1, "yyyyMMddHHmmssSSS", "yyyyMMddHHmmssSSS");
		...
		int dateDef = DateUtil.getDateDiff(DateUtil.getNow("yyyyMMddHHmmssSSS"), DateUtil.dateAdd(13, 30, "yyyyMMddHHmmssSSS", "yyyyMMddHHmmssSSS"), 1000, "yyyyMMddHHmmssSSS");
